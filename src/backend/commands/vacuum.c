...
expand_vacuum_rel(VacuumRelation *vrel, int options)
{
	...
	else
	{
		...
		/*
		 * In addition to expanding a partition table to consider inerhitance,
		 * we also use this function to expand appendoptimized tables to their
		 * auxiliary tables if the statement indicates vacuuming those. This
		 * is something of a misnomer because we do not pass the originally
		 * input AO table, so it's a replacement instead of strictly an
		 * expansion.
		 */
		if ((options & VACOPT_AO_AUX_ONLY) != 0)
		{
			...
			foreach (lc, vacrels)
			{
				...
				if (IsAccessMethodAO(classForm->relam))
				{
					...
					// make new VacuumRelations for each of the 3 auxiliary tables
					VacuumRelation *aoseg_vrel = makeVacuumRelation(NULL, aoseg_relid, part_vrel->va_cols);
					VacuumRelation *aoblkdir_vrel = makeVacuumRelation(NULL, aoblkdir_relid, part_vrel->va_cols);
					VacuumRelation *aovisimap_vrel = makeVacuumRelation(NULL, aovisimap_relid, part_vrel->va_cols);
					ao_aux_vacrels = lappend(ao_aux_vacrels, aoseg_vrel);
					ao_aux_vacrels = lappend(ao_aux_vacrels, aoblkdir_vrel);
					ao_aux_vacrels = lappend(ao_aux_vacrels, aovisimap_vrel);
				}
				ReleaseSysCache(tuple);

			}

			// return only the AO AUX vacrels, silently dropping any heap tables input entirely
			// TODO -- consider an else+warning here so it isn't silent?
